...
const FROM_EMAIL = "SyncSpace <noreply@syncspace.com>";
...
export async function sendProjectInvitationEmail(
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      to: [email],
      subject,
      text: `${inviterName} invited you to join "${projectName}" on SyncSpace. Accept here: ${invitationUrl}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2>Project Invitation</h2>
          <p><strong>${inviterName}</strong> has invited you to join the project <strong>"${projectName}"</strong> on SyncSpace.</p>
...
export async function sendSubsEmail(email: string): Promise<void> {
  const SyncSpaceUrl = `${BASE_URL}`;
  const subject = `Subscription to SyncSpace`;
  ...
  try {
    const { data, error } = await resend.emails.send({
      ...
      to: [email],
      subject,
      text: `Thank you for subscribing to SyncSpace! You can start using it here: ${SyncSpaceUrl}`,
      html: `
        <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
          <h2>Subscription Confirmation</h2>
          <p>Thank you for subscribing to SyncSpace! We're excited to have you on board.</p>
          <p>You can start using SyncSpace by clicking the button below:</p>
          <a href="${SyncSpaceUrl}" style="display:inline-block;padding:10px 20px;background:#4f46e5;color:white;text-decoration:none;border-radius:5px;">SyncSpace Home</a>
          <p>If the button doesn't work, copy and paste this link: ${SyncSpaceUrl}</p>
        </div>
      `,
    });
